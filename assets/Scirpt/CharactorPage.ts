...
import { ExtraPoint, SetInfo, UserData } from "./DataBase";
...
export default class CharactorPage extends BaseSingletonComponent<CharactorPage>() {
    ...
    UserDataLoad() {
        ...
        data[0] = SetInfo.getInstance.setUserInfo(data[0] as UserData, data[1] as ExtraPoint);
        for (let i of [`AD`, `AP`, `DEF`, `MDF`, `Speed`]) {
            this.conUserData[i].string = data[0][i].toString();
        }
        for (let i of [`Dodge`, `Critical`]) {
            this.conUserData[i].string = Number(data[0][i]).toFixed(3);
        }
    }
